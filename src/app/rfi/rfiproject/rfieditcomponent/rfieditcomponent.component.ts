...
@Component({
  selector: 'app-rfieditcomponent',
  templateUrl: './rfieditcomponent.component.html',
  styleUrls: ['./rfieditcomponent.component.css']
})
export class RFIEDITCOMPONENTComponent {
  ...
  toggleRecipientEdit() {
    ...
    if (this.isRecipientEditable) {
      this.projectRfiControl.enable();
    } else {
      this.projectRfiControl.disable();
    }
  }
     ...
     if(formData){
       ...
       if(parsedData){
         ...
         this.rfiForm.patchValue({
           ...
           dueDate: parsedData.dueDate ? parsedData.dueDate.toDate(): '', 
    ...
    this.data_api.getFBProject(this.passID.id).subscribe(data => {
      console.log('res', data)
        ...
        if(data.recipientRFI){
          ...
          console.log('this.setProjectRfiRecipient', this.setProjectRfiRecipient)
          console.log('clientmeial', this.rfiForm.value.clientEmail)
        }

          // EMAIL CCS
          if (data.clientEmailCC) {
            this.clientCCs = data.clientEmailCC
          }
          
          // EMAIL BCCS
          if (data.clientEmailBCC) {
            this.clientBCCs = data.clientEmailBCC
          }
  ...
  handleEmailAdd(event: KeyboardEvent, type: 'cc' | 'bcc') {
    ...
    if (value && checkValidEmail(value, swal)) {
      ...
      const targetArray = type === 'cc' ? this.clientCCs : this.clientBCCs;
    ...
    console.log('clientCCs:', this.clientCCs);
  }
  ...
  private updateEmailList(emailList: string[], type: 'cc' | 'bcc') {
    ...
    this.data_api.updateCCsAndBCCs(emailList, type, this.projectData.id)
  ...
  public getRFI(){
    this.data_api.getFBAFI(this.passID.id2).subscribe(data => {
        ...
        this.rfiForm.patchValue({
          ...
          // bmLineitem: this.checkBooleanVariationSettings(data.bmLineitem) ? data.bmLineitem : (this.checkGlobalBooleanVariationSettings(this.rfiAdminData.bmLineitem) ? this.rfiAdminData.bmLineitem : false),
          // bmTotalFigure: this.checkBooleanVariationSettings(data.bmTotalFigure) ? data.bmTotalFigure : (this.checkGlobalBooleanVariationSettings(this.rfiAdminData.bmTotalFigure) ? this.rfiAdminData.bmTotalFigure : false),
          // bmHideAll: this.checkBooleanVariationSettings(data.bmHideAll) ? data.bmHideAll : (this.checkGlobalBooleanVariationSettings(this.rfiAdminData.bmHideAll) ? this.rfiAdminData.bmHideAll : false),
          // qtyHideAll: this.checkBooleanVariationSettings(data.qtyHideAll) ? data.qtyHideAll : (this.checkGlobalBooleanVariationSettings(this.rfiAdminData.qtyHideAll) ? this.rfiAdminData.qtyHideAll : false),
          // unitHideAll: this.checkBooleanVariationSettings(data.unitHideAll) ? data.unitHideAll : (this.checkGlobalBooleanVariationSettings(this.rfiAdminData.unitHideAll) ? this.rfiAdminData.unitHideAll : false),
          // unitCostHideAll: this.checkBooleanVariationSettings(data.unitCostHideAll) ? data.unitCostHideAll : (this.checkGlobalBooleanVariationSettings(this.rfiAdminData.unitCostHideAll) ? this.rfiAdminData.unitCostHideAll : false),
          // gstHideAll: this.checkBooleanVariationSettings(data.gstHideAll) ? data.gstHideAll : (this.checkGlobalBooleanVariationSettings(this.rfiAdminData.gstHideAll) ? this.rfiAdminData.gstHideAll : false),
  ...
  public copyPdfLink(){
    this.clipboard.copy(this.rfiForm.value.pdfLink);
 ...
 onProjectRFiToppingRemoved(topping, index:number){
  console.log('thisproectrficontrol' ,this.projectRfiControl.value)
  const newToppings = this.projectRfiControl.value.filter((_, i) => i !== index);
  this.projectRfiControl.patchValue(newToppings);
  ...
  onToppingRemoved(topping, index: number) {
    ...
    addRecipient(event: KeyboardEvent): void {
      ...
      if (input) {
        ...
        // Ensure the value is an array before pushing the newRecipient
    ...
    onSelectionChange(event?: any): void {
        try {
          ...
          const recipientIds = this.projectRfiControl.value.map((recipient: any) => recipient.id);
  ...
  public findInvalidControls() {
          ...
          if (controlsRfi[name].invalid) {
            if (controlsRfi[name].invalid) {
                if(name == 'rfiName'){
                  ...
                  invalid.push('One-time Rfi Recipients');
      ...
      swal.fire({
          title: "Please fill required fields! 1",
          html: htmlVal,
...
getFooter2(currentPage, pageCount) {
  ...
  public saveStep1Confirmation(message){
      ...
      swal.fire({
          // title: 'Are you sure?',
          text: "This will override this RFI and will require the client approve it again. Are you sure you want to do this?",
          icon: 'warning',
  ...
  public async saveStepCheckValidation(message,action){   // Generate and Upload PDF to Firebase  
    ...
    //console.log('rfiform',this.rfiForm.value);
        ...
        console.log('project orgwec', this.rfiForm.value.projectOwner)
        console.log('2',this.setProjectRfiRecipient)
  ...
  sendAdminEmail(variantID,projectID){
    ...
    let adminEmail = this.rfiAdminData.rfiEmailRecipient;
    if(adminEmail){
    ...
    let rep2 = '/rfi/project/'+projectID+'/edit/'+variantID ;
    ...
    let tempdata = {
      ...
      varLink: newUrl,
      pdfLink: this.rfiForm.value.pdfLink,
...
sendClientEmail(variantID,projectID){
    ...
    // THIS IS One-time RFI Recipient/s
    ...
    let emailsCC = [...this.projectData.clientEmailCC];
    ...
    let emailsBCC = [...this.projectData.clientEmailBCC]
    if(emailsBCC){
    ...
    let myURL = window.location.href ;
    let rep2 = '/dashboard-rfi/'+variantID ;
    let rep1 = this.router.url ;
    let newUrl = myURL.replace(rep1, rep2)
...
btnFunctionCancel(){
  ...
  if(this.rfiForm.dirty){
        ...
        swal.fire({
          ...
          if (result.isConfirmed) {
            this.router.navigate(['/rfis/project/'+this.passID.id]);
          } 
...
public findInvalidControls() {
  ...
  swal.fire({
      title: "Please fill required fields! 2",
      html: htmlVal,
      buttonsStyling: false,
      customClass: {
        confirmButton: 'btn btn-success',
...
public findInvalidControls() {
    ...
    swal.fire({
        title: "Please fill required fields! 3",
        html: htmlVal,
        buttonsStyling: false,
        customClass: {
          confirmButton: 'btn btn-success',
...
@Component({
selector: 'rfis-edit-user-add-owner-dialog',
...
public addNewUom() {
    ...
    if (this.rfiForm.invalid) {
      ...
      swal.fire({
          title: "Please fill required fields! 4",
          // text: "You clicked the button!",
          buttonsStyling: false,
          customClass: {
            confirmButton: 'btn btn-success',
...
public async confirmApprove() {
    ...
    if (this.rfiForm.controls['comments'].invalid){
        swal.fire({
    ...
    if(this.rfiForm.controls['approveRfi'].value != true){
        swal.fire({
            title: "Please acknowledge and approve the proposed rfi",
            buttonsStyling: false,
            customClass: {
              confirmButton: 'btn btn-success',
            },
...
ngOnInit() {
  ...
  this.rfiForm = this.formBuilder.group({
    ...
    approveRfi: ['', Validators.required],
  }, {
  });
...
public addNewGroupName() {
    ...
    if (this.rfiForm.invalid) {
      swal.fire({
          title: "Please fill required fields! 5",
          // text: "You clicked the button!",
          buttonsStyling: false,
          customClass: {
            confirmButton: 'btn btn-success',
...
public createFBVarGroupName(): void {
    ...
    if (this.rfiForm.invalid) {
      ...
      swal.fire({
          title: "Please fill required fields! 6",
          // text: "You clicked the button!",
          buttonsStyling: false,
          customClass: {
            confirmButton: 'btn btn-success',

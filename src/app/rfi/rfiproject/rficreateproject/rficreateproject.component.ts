...
import {ENTER, COMMA, V, I} from '@angular/cdk/keycodes';
...
@Component({
  selector: 'app-rficreateproject',
  templateUrl: './rficreateproject.component.html',
  styleUrls: ['./rficreateproject.component.css']
})
export class RFICREATEPROJECTComponent {
  rfiForm: FormGroup;
  ...
  rfiEmailRecipient : ReplaySubject<[]> = new ReplaySubject<[]>(1);

  projectRFIRecipientControl = new FormControl({ value: [], disabled: true });

  public setProjectRFIRecipient :any = [];
    ...
    this.deviceInfo = this.deviceService.getDeviceInfo();
     ...
     this.passID = {
      ...
      this.rfiForm = this.formBuilder.group({
          ...
          projectOwner: [''],
          rfiGroupArray: this.formBuilder.array([]),
      ...
      // let _rfiNumber = await this.getFBCounterRFI();


      this.rfiForm.patchValue({
        projectId:  this.passID.id,
        // rfiNumber: _rfiNumber
      });
  ...
  isRecipientEditable: boolean = false;

  toggleRecipientEdit() {
    this.isRecipientEditable = !this.isRecipientEditable;
  
    if (this.isRecipientEditable) {
      this.projectRFIRecipientControl.enable();
    } else {
      this.projectRFIRecipientControl.disable();
    }
  }
  

  onSelectionChange(event?: any): void {
    console.log('rfi form', this.rfiForm.value)
    try {
      // Extract recipient IDs from the selected values
      const recipientIds = this.projectRFIRecipientControl.value.map((recipient: any) => recipient.id);
    ...
    setTimeout(()=>{
     if(formData){
       ...
       if(parsedData){
        ...
        //    projectOwnerIDs = parsedProjectOwnerControl;
        ...
        //   }       
         ...
         if (parsedData.rfiGroupArray){
           ...
           const updatedArray = parsedData.rfiGroupArray.map(item => {
            const { tempFiles, ...rest } = item;
            return rest;
            });
           
           this.rfiGroupArray().patchValue(updatedArray)    
       }
      }
     }
    },4000)
  ...
  getProject(){
    ...
    // this.data_api.getFBProject(this.passID.id).pipe(first()).subscribe(data => 
        ...
        let rfiSetJobNum =  this.rfiAdminData.rfiSetJobNum ? this.rfiAdminData.rfiSetJobNum : true;
        let rfiSetCode =  this.rfiAdminData.rfiSetCode ? this.rfiAdminData.rfiSetCode : 'RFI';
        ...
        let displayJobNum = ''
        if(rfiSetJobNum == true){
          displayJobNum = this.projectData.jobNumber;
        }


        if(this.projectData.counterRFI){ //if project variation exist

          if(this.projectData.counterRFI >= varSetStartNumber){ //if project var num is higher or equal than admin var num

            this.displayRfiNumber =  this.checkDigitNumbers(parseInt(this.projectData.counterRFI) + 1);

          }else{ //if project var num is lower than admin var num

            if(varSetStartNumber > 1){  //if admin var num is higher than 0
              this.displayRfiNumber = this.checkDigitNumbers(varSetStartNumber);
            }else{
              this.displayRfiNumber = this.checkDigitNumbers(1);
            }
        ...
        this.rfiForm.patchValue({
          rfiNumber: displayJobNum + rfiSetCode + this.checkDigitNumbers(this.displayRfiNumber ),
        ...
        // if(this.projectData.counterRFI){
        //     this.rfiForm.patchValue({
        //       rfiNumber: displayJobNum + rfiSetCode + this.checkDigitNumbers( parseInt(this.projectData.counterRFI) + 1 )
        //     });
        // }else{
        //     this.rfiForm.patchValue({
        //       rfiNumber: data.jobNumber + "V0001"
        //     });
        //     this.projectData.counterRFI = 0;
        // } 
        ...
        if(data.recipientRFI){
          ...
          projectOwnerIDs.forEach(value => {
              if(this.findObjectByKey(this.projectOwnersProject, 'id', value)){
                  ...
                  this.setProjectRFIRecipient.push(item);
              }
          ...
          this.rfiEmailRecipient.next(this.setProjectRFIRecipient)
          this.projectRFIRecipientControl.setValue(this.setProjectRFIRecipient);
        ...
        // EMAIL CCS
        if (data.rfiEmailCC) {
          this.rfiEmailCC = data.rfiEmailCC
        }
        
        // EMAIL BCCS
        if (data.rfiEmailBCC) {
          this.rfiEmailBCC = data.rfiEmailBCC
        }
  ...
  rfiGroupArray(): FormArray {
    return this.rfiForm.get('rfiGroupArray') as FormArray;
  }
  ...
  // async getFBCounterRFI(){
  //   const data =  await this.data_api.getFBCounterRFI().pipe(take(1)).toPromise();
  //   if(data){
...
//   }
  ...
  onToppingRemoved(topping, index: number) {
    ...
    public findInvalidControls() {
      const invalid = [];
      const controlsRfi = this.rfiForm.controls

      for (const name in controlsRfi) {
          if (controlsRfi[name].invalid) {
              if(name == 'rfiName'){
                invalid.push('RFI Name');
  ...
  getFooter2(currentPage, pageCount) {
    ...
    //     let accsList = this.rfiForm.value.rfiGroupArray;
    ...
    rfiSettingsWidth(){
          ...
          if(this.rfiForm.value.qtyHideAll == true){ //Qty
            descWidth = descWidth + 10;
          }else{
            bulletWidths.push('10%');
          }

          if(this.rfiForm.value.unitHideAll == true){  //UOM
            descWidth = descWidth + 10;
          }else{
            bulletWidths.push('10%');
          }

          if(this.rfiForm.value.unitCostHideAll == true){ //Cost
            descWidth = descWidth + 10;
          }else{
            bulletWidths.push('10%');
          }

          if(this.rfiForm.value.bmHideAll == true){ //BM
            descWidth = descWidth + 10;
          }else{
            if(this.rfiForm.value.bmLineitem == true){
              bulletWidths.push('10%');
            }else{
              descWidth = descWidth + 10;
            }
          ...
          if(this.rfiForm.value.itemTotalHideAll == true){ //Subtotal
            descWidth = descWidth + 10;
          }else{
            bulletWidths.push('10%');
          }

          // if(this.rfiForm.value.gstHideAll == true){ //GST
          //   descWidth = descWidth + 10;
          // }else{
          //   bulletWidths.push('10%');
          // }

          // if(this.rfiForm.value.itemTotalHideAll == true){ //Total
          //   descWidth = descWidth + 10;
          // }else{
          //   bulletWidths.push('10%');
          // }
    ...
    //   // if(this.rfiForm.value.itemTotalHideAll == true){ //Subtotal Total
    //   //   // descWidth = descWidth + 10;
    //   // }else{
    ...
    //   let newArr = [{text:  'Summary', style: 'tableHeader'}, ...bulletTitle];
    ...
    pdfBmHideAllTitleWidth(){
      if(this.rfiForm.value.bmHideAll == true){
  ...
  pdfBmHideAll2(bmGroupTotal){
    if(this.rfiForm.value.bmHideAll == true){
      ...
      if(this.rfiForm.value.bmTotalFigure == true){
        return [
                  {
                    text: '', 
                    style: 'tableHeader',
  ...
  getSummary(rfiName){
    if(rfiName){
      return[
        htmlToPdfmake('<b>Summary</b>'),
        htmlToPdfmake(rfiName)
      ]
    }
  }
  ...
  getClosingMessage(closingMessage) {
    if (closingMessage) {
      return [
        htmlToPdfmake('<b>Detail</b>'),  
        htmlToPdfmake(closingMessage)
      ];
    }
    return [];
  }
              ...
              columns: [
                ...
                },
                {
                  stack: [
                    ...
                    {
                      text: this.rfiForm.value.rfiNumber, //(this.editForm.value.reportNumber ? this.editForm.value.reportNumber : ''),
                      style: 'fieldData',
                    }
                  ],
                }
    ...
    content: [
      ...
      // { text: 'To: '+this.splitArray(this.pdfClientNames), style: 'fieldHeader', margin: [0, 0, 0, 20 ],},
      {
        columns: [
          ...
          {
            text:  this.rfiForm.value.rfiNumber,
            style: 'fieldHeader',
            width: '32%',
          }
        ],
        ...
        columns: [
          ...
          
          {
            stack: [
              ...
              {
                text: moment(this.rfiForm.value.dueDate).locale("en-au").format('LL'),
                style: 'fieldData',
              },
              {
      ...
      this.getSummary(this.rfiForm.value.rfiName),
      { text: '', style: 'fieldHeader', margin: [0, 0, 0, 20 ],},
      this.getClosingMessage(this.rfiForm.value.closingMessage),
      {
        columns: [
          {
            text: 'Approved by:',
      ...
      }, 
      {
        columns: [
          ...
          },
          {
            stack: [
              {
                text: ' ', //image: this.rfiForm.value.signature,
                width: '150',
                margin: [ 15, 30, 0, 0 ]
              },
...
oneTimeRecipientsInputControl = new FormControl('');


addRecipient(event: KeyboardEvent): void {
  event.stopPropagation();
  event.preventDefault();
  const input = this.oneTimeRecipientsInputControl.value;
  if (!input) {
    return;
  }
  ...
  if (input) {
    const newRecipient = { 
      email: input,
      id: '123'};  // this is static for error
    const currentValue = this.projectOwnerControl.value;
    
    // Ensure the value is an array before pushing newRecipient
    if (Array.isArray(currentValue)) {
      this.projectOwnerControl.setValue([...currentValue, newRecipient]);
    }
    console.log('this.rifform', this.rfiForm);

    // Clear the input field after adding the recipient
    this.oneTimeRecipientsInputControl.setValue('');  // Clear the input control
  }
    // Now update the clientEmail field in rfiForm
      ...
      console.log("projectOwnerControl", this.projectOwnerControl.value)
...
handleEmailAdd(event: KeyboardEvent, type: 'cc' | 'bcc') {
  ...
  if (value && checkValidEmail(value, swal)) {
    ...
    const targetArray = type === 'cc' ? this.rfiEmailCC : this.rfiEmailBCC;
  ...
  console.log('rfiEmailCC:', this.rfiEmailCC);
}
...
private updateEmailList(emailList: string[], type: 'cc' | 'bcc') {
  this.spinnerService.show();
  this.data_api.updateCCsAndBCCs(emailList, type, this.projectData.id, 'rfi')
...
toggleEdit(type: 'cc' | 'bcc') {
  ...
  saveStepCheckValidation(message?: string){
      ...
      if(this.rfiForm.value.rfiGroupArray.length < 1 ){
        swal.fire({
      ...
      if(this.rfiForm.value.rfiGroupArray ){
        let errorDetect = 0;
        let imgCount = 0;
        for (let group of this.rfiForm.value.rfiGroupArray) { 
          for (let item of group.itemArray) {
      ...
      if(  (this.rfiForm.value.projectOwner.length < 1) &&  (this.setProjectRFIRecipient.length < 1) ){

        swal.fire({
          title: "Please Add at least 1 RFI Recipient",
              // text: htmlVal,
              buttonsStyling: false,
              customClass: {
                confirmButton: 'btn btn-success',
      ...
      if(tobeuploadPDFs > 0){
        ...
        this.saveStepUploadPDF(message);
      }
  ...
  saveStepUploadQuotes(){
    ...
    let pdfDone = 0;
    let i = 0;
    //let imageLen = this.rfiForm.value.rfiGroupArray.length;
    let pdfLen = 0;
    for (let group of this.rfiForm.value.rfiGroupArray) { 
      if(group.tempFiles){
        for (let file of group.tempFiles) {
          ...
          for (let group of this.rfiForm.value.rfiGroupArray) { 
            ...
            if(group.tempFiles){
                  ...
                  //for (let i = 0; i < this.imageURL.length; i++) {
                    ...
                    let fileName = this.rfiForm.value.rfiNumber+'-'+groupIndex+'-'+fileIndex+'-'+id+'.pdf';
                    let ref = this.afStorage.ref(this.accountFirebase+'/'+this.projUploadFolder+'/Rfis/'+folderName+'/'+fileName);
                    ...
                    task.snapshotChanges().pipe(
                      finalize(async () => {
                        ...
                        ref.getDownloadURL().subscribe((url) => { 
                          ...
                          myFiles.push(url);
                          let newFiles = [];
                          if(this.rfiGroupArray().at(myIndex).get('files').value){
                            newFiles = myFiles.concat(this.rfiGroupArray().at(myIndex).get('files').value);
                          }else{
                            newFiles = myFiles;
                          }
                          ...
                          this.rfiGroupArray().at(myIndex).get('files').patchValue(newFiles);
  ...
  previewPdf(){
    this.saveStepUploadPDFwithQuotes(true);
  }

  async saveStepUploadPDFwithQuotes(preview?: boolean){
      let myFiles = [];
      ...
      pdfDocGenerator.getBuffer(async (buffer) => {
          ...
          copiedPagesA.forEach((page) => mergedPdf.addPage(page));
            ...
            let folderName =  this.rfiForm.value.folderName;  
            let id = this.rfiForm.value.rfiNumber+'.pdf';
  ...
  public downloadPDF(pdfBytes: any, fileName: string) {
    // Create a blob URL from the pdfBytes
  ...
  //       .then(blob => {
  //         const blobUrl = window.URL.createObjectURL(blob); // Create a local URL
  //         const link = document.createElement('a');
  //         link.href = blobUrl;
  //         link.download = fileName;
  //         document.body.appendChild(link);
  ...
  async saveStepUploadPDF(message?:string){
      this.progressOverlay.show('Uploading RFI PDF','#0771DE','white','lightslategray',1);
      const documentDefinition = this.getDocumentDefinition();
        ...
        for (let group of this.rfiForm.value.rfiGroupArray) { 
          for (let item of group.itemArray) {
            ...
            let id = Math.random().toString(36).substring(2);
              ...
              let ref = this.afStorage.ref(this.accountFirebase+'/'+this.projUploadFolder+'/Rfis/'+folderName+'/'+i);
  ...
  async saveStepCreateRfi(){
      ...
      let tempIndex = 0;
      for (let group of this.rfiForm.value.rfiGroupArray) { 
        this.rfiGroupArray().at(tempIndex).get('tempFiles').disable();
        tempIndex++;
      }
      ...
      this.data_api.updateFBCounterProjectRFI(this.rfiForm.value.projectId, this.displayRfiNumber ).then(() => {
           console.log('RFI Counter incremented!');
      });
      ...
      this.data_api.createFBRFI(this.rfiId,this.rfiForm.value).then(data => {
        ...
        this.router.navigate(['/rfi/project/'+this.passID.id+'/edit/'+this.rfiId]);  
  ...
  ownerSelectChange(event)
  {
    ...
    if(event.isUserInput) {
        ...
        if(event.source.selected == true){
            ...
            let tempArray = [event.source.value.email, ...this.rfiForm.value.clientEmail];
              this.rfiForm.patchValue({
            ...
            let tempArray = this.rfiForm.value.clientEmail;
  ...
  onProjectRFiToppingRemoved(topping, index:number){
    console.log('thisproectrficontrol' ,this.projectRFIRecipientControl.value)
    console.log('this.rfiform', this.rfiForm.value);
    const newToppings = this.projectRFIRecipientControl.value.filter((_, i) => i !== index);
    this.projectRFIRecipientControl.patchValue(newToppings);
  ...
  sendAdminEmail(variantID,projectID){
    ...
    let clientEmail = this.rfiForm.value.clientEmail;
    if(clientEmail){
    ...
    callableTest(tempdata).subscribe(result => {
      ...
      this.router.navigate(['/rfi/project/'+projectID]);
...
toggleBMLineitem(){
  ...
  if(this.rfiForm.value.bmLineitem){
      this.rfiForm.patchValue({
        bmHideAll: false,
      });
  }
...
toggleBMTotalFigure(){
  ...
  if(this.rfiForm.value.bmTotalFigure){
      this.rfiForm.patchValue({
        bmHideAll: false,
      });
  }
...
toggleBMHideAll(){
  ...
  if(this.rfiForm.value.bmHideAll){
      this.rfiForm.patchValue({
        bmLineitem: false,
        bmTotalFigure: false
      });
...
openAddOwnerDialog(): void {
  ...
  dialogRef.afterClosed().subscribe(result => {
      if(result == 'success'){   
          ...
          // }, 1000);  
        //  this.getProjectOwners();TableItemsAddDialog
      }
  });
}
...
btnFunctionCancel(){
  ...
  if(this.rfiForm.dirty){
        ...
        swal.fire({
          ...
          if (result.isConfirmed) {
            this.router.navigate(['/rfi/project/'+this.passID.id]);
          } 
...
public addNewItem() {
  ...
  this.dialogRef.close(this.rfiForm.value);
...
public onComputeTotal(){
    ...
    let _quantity = this.rfiForm.value.quantity;
    let _unitCost = this.rfiForm.value.unitCost;

    let _subtotal = _quantity *  _unitCost;
    let _taxRate = 10;
    let _bmpercentage = this.rfiForm.value.buildersMargin;
    let _bmp_subtotal = (_bmpercentage * _subtotal) / 100;
    let _gst = ((_subtotal + _bmp_subtotal) * _taxRate) /  100;
    let _itemTotal = _gst + _bmp_subtotal + _subtotal;
...
public findInvalidControls() {
    const invalid = [];
    const controlsRfi = this.editForm.controls;
...
public checkFBUserExist(): void {
    ...
    this.data_api.checkFBUserExist(this.rfiForm.value.userEmail).pipe(first()).subscribe(data => {
        ...
        if(data.length > 0){
          ...
          this.rfiForm.controls['userEmail'].reset()
...
public addNewUserOwner() {
    ...
    if (this.rfiForm.value.password.length < 6) {
      swal.fire({
        ...
        this.data_api.getFBVarGroupNames().subscribe(data => {
            if(data){
              if(data.nameArray){
                ...
                this.listGroupNames = data.nameArray;
                this.rfiForm.get("groupName").patchValue(result.toString());
                this.initializeFilterGroupNames();
              }
            }
        }); 
...
public addLog(id){
    ...
    let passData = {
        todaysDate: today,
        log: 'Created New RFI Group Name - Global List',
        method: 'create',
        subject: 'groupname',
        subjectID: id,
        data: this.rfiForm.value,

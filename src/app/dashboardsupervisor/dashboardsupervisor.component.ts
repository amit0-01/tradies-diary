...
export class DashboardSupervisorComponent {
  ...
  latestDailyReportsLogsOptions = {...this.dataTableOptions}
  dtTriggerDailyReportsLogs:  Subject<any> = new Subject();
  latestProjectsCreatedOptions = {...this.dataTableOptions};
  dtTriggerProjectTable : Subject<any> = new Subject();
  latestWeeklyReportOptions = {...this.dataTableOptions}
  dtTriggerWeeklyReport : Subject<any> = new Subject();
  latestWokerEntryLogsOptions= {...this.dataTableOptions}
  dtTriggerWorkerEntryLog  : Subject<any> = new Subject();
...
//   };
  ...
  public ngOnInit() {
   ...
   ngOnDestroy(){
    this.dtTriggerDailyReportsLogs.unsubscribe();
    this.dtTriggerProjectTable.unsubscribe();
    this.dtTriggerWeeklyReport.unsubscribe();
    this.dtTriggerWorkerEntryLog.unsubscribe();
   }
  ...
  getFBRecent(){
    ...
    this.data_api.getFBRecentDailyReportSupervisor(this.userDetails.user_id).subscribe(dataDailyReports => {
        ...
        if(dataDailyReports){
            ...
            this.setTableDataForDailyReportList(this.dashboardDailyReportList);
            dataDailyReports.forEach(data =>{ 
              if(data.imageUpload){
    ...
    this.data_api.getFBRecentWeeklyReportSupervisor(this.userDetails.user_id).subscribe(dataWeeklyReports => {
   ...
   setTableDataForDailyReportList(data) {
    if ($.fn.DataTable.isDataTable('#dailyReportTable')) {
      $('#dailyReportTable').DataTable().destroy();
    }
    this.latestDailyReportsLogsOptions.data = data
    ...
    setTimeout(()=>{
      this.dtTriggerDailyReportsLogs.next();
      console.log('datatable is triggered');
    },1000)
  }
...
setTableDataForProjectsList(){
  ...
  this.latestProjectsCreatedOptions.columns = [
    ...
    setTimeout(()=>{
      this.dtTriggerProjectTable.next();
      console.log('datable is triggered')
    },1000)
}
...
setTableDataForWeeklyReport() {
  ...
  this.latestWeeklyReportOptions.columns = [
      ...
      },
      {
        ...
        searchable: false,
        render: (data, type, row) => 
          this.projectNames.find(o => o.id === row?.projectId)?.projectName ?? ''
      },
      {
        title: 'Entry Date',
        width: '150px',
        ...
        searchable: false,
        render: (data, type, row) => 
          row?.weekendDate?.toDate()?.toDateString() ?? ''
      },
      {
        data: 'imageUpload',
        title: 'No. Images',
    ...
    setTimeout(()=>{
      this.dtTriggerWeeklyReport.next();
      console.log('datable is triggered')
    },1000)
}
setTableDataForWorkerEntryLogs(data){
  if ($.fn.DataTable.isDataTable('#workerEntryTable')) {
    $('#workerEntryTable').DataTable().destroy();
  }
  this.latestWokerEntryLogsOptions.data = data, 
  ...
  setTimeout(() => {this.dtTriggerWorkerEntryLog.next()
     console.log('datatable is triggered')
  }, 1000);
}
...
// SETTING DATATABLE DATA CLOSE 
  ...
  async  getFBRecentWorkerEntryLogsSupervisor(){
      ...
      this.dashboardWorkerList = batches;
      this.setTableDataForWorkerEntryLogs(this.dashboardWorkerList);

...
export class SelectionsprojectcreateComponent implements OnInit {
  ...
  selectionForm: FormGroup;
    ...
    projectSelectionRecipientControl = new FormControl({value : [], disabled: true});
    ...
    // SELECTION CHANGE OF PROJECT SELECTION RECEIPIENT
    
    onSelectionChange(event?: any): void {
        try {
          // Extract recipient IDs from the selected values
          const recipientIds = this.projectSelectionRecipientControl.value.map((recipient: any) => recipient.id);
      ...
      onToppingSelectionRemoved(topping, index:number){
        console.log('thisproectrficontrol' ,this.projectSelectionRecipientControl.value)
        const newToppings = this.projectSelectionRecipientControl.value.filter((_, i) => i !== index);
        this.projectSelectionRecipientControl.patchValue(newToppings);
         ...
         if(parsedData){
          ...
          //  }
           ...
           if (parsedData.selectionGroupArray){
             console.log(parsedData.selectionGroupArray);
    ...
    saveForm(){
      ...
      localStorage.setItem('formData',JSON.stringify(this.selectionForm.value))
      }
      ...
      this.data_api.getFBProject(this.passID.id).subscribe((data) => {
        console.log('data', data);
          ...
          // this.data_api.getFBProject(this.passID.id).pipe(first()).subscribe(data => {
              ...
              let selSetJobNum = this.selectionAdminData.selSetJobNum ? this.selectionAdminData.selSetJobNum : true;
              let selSetCode = this.selectionAdminData.selSetCode ? this.selectionAdminData.selSetCode : 'V';
              let selSetStartNumber = this.projectData.selProjStartNumber ? this.projectData.selProjStartNumber : 1;
  
              let displayJobNum = ''; 
              if (selSetJobNum == true) { 
                  displayJobNum = this.projectData?.jobNumber ;
              }
  
              if (this.projectData.counterSelection) { // if project variation exists
                  if (this.projectData.counterSelection >= selSetStartNumber) { // if project var num is higher or equal to admin var num
                      this.displaySelectionNumber = this.checkDigitNumbers(parseInt(this.projectData.counterSelection) + 1);
                  } else { // if project var num is lower than admin var num
                      if (selSetStartNumber > 1) {  // if admin var num is higher than 0
                          this.displaySelectionNumber = this.checkDigitNumbers(selSetStartNumber);
                      } else {
              ...
              this.selectionForm.patchValue({
                  selectionNumber: displayJobNum + selSetCode + this.checkDigitNumbers(this.displaySelectionNumber),
                  ...
                  bmHideAll: this.checkGlobalBooleanSelectionSettings(data.bmHideAll) ? data.bmHideAll : false,
                  ...
                  unitCostHideAll: this.checkGlobalBooleanSelectionSettings(data.unitCostHideAll) ? data.unitCostHideAll : false,
                  gstHideAll: this.checkGlobalBooleanSelectionSettings(data.gstHideAll) ? data.gstHideAll : false,
                  itemTotalHideAll: this.checkGlobalBooleanSelectionSettings(data.itemTotalHideAll) ? data.itemTotalHideAll : false,
              });
  ...
  toggleRecipientEdit() {
    ...
    if(this.isEditable){
      this.projectSelectionRecipientControl.enable()
    } else{
      this.projectSelectionRecipientControl.disable();
    }
  }
    ...
    // async getFBCounterSelectioncounterSelection(){
    //   const data =  await this.data_api.getFBCounterSelectioncounterSelection().pipe(take(1)).toPromise();
    //   if(data){
  ...
  computeGroupTotal(groupIndex){
    ...
    onToppingRemoved(topping, index: number) {
      ...
      public findInvalidControls() {
        const invalid = [];
        const controlsSelection = this.selectionForm.controls;
...
cleanChar(str){
  if(str){
    ...
    async saveStepCreateSelection(){
        ...
        this.data_api.updateFBCounterProjectSelection(this.selectionForm.value.projectId, this.displaySelectionNumber ).then((res) => {
            console.log('res', res)
        });
...
export class TableCreateItemsEditDialog implements OnInit {
  ...
  public findInvalidControls() {
      const invalid = [];
      const controlsSelection = this.editForm.controls;
...
@Component({
  selector: 'selections-create-user-add-owner-dialog',

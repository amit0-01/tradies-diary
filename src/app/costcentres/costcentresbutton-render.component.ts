...
export class CostcentresRenderComponent implements ViewCell, OnInit {
 ...
 public prevdata;
  ...
  showSwal() {
    ...
    openDialog(): void {
        console.log(this.renderValue);
        const dialogRef = this.dialog.open(CostcentresDialog, {
            width: '400px',
    ...
    openDeleteDialog(): void {
      ...
      const dialogRef = this.dialog.open(CostcentresDeleteDialog, {
          width: '400px',
          data: this.renderValue
      });
        ...
        console.log(result);
          ...
          if(result){ 
              ...
              this.spinnerService.show();   
                  console.log(result);
                  this.data_api.deleteFBCostcentre(result).then(() => {
                    this.addLog();
                    $.notify({
                      icon: 'notifications',
                      message: 'Cost Centre Deleted'
                    }, {
                        type: 'success',
                        timer: 1000,
                        placement: {
    ...
    addLog(){
        ...
        let passData = {
            todaysDate: today,
            log: 'Deleted Cost Centre - Global List',
            method: 'delete',
            subject: 'costcentre',
            subjectID: this.renderValue.costcentreID,
...
@Component({
    selector: 'costcentres-dialog',
    templateUrl: 'costcentresdialog.html',
    providers: [
      {provide: DateAdapter, useClass: AppDateAdapter},
      {provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS}
    ]
  })
export class CostcentresDialog implements OnInit {
    ...
    constructor(
      ...
      public dialogRef: MatDialogRef<CostcentresDialog>,
    ...
    ngOnInit() {
        ...
        this.editForm.patchValue({
          costcentreID: this.data.costcentreID,
          costcentreName: this.data.costcentreName,
        });
    ...
    //     let passData = {
    //         todaysDate: today,
    //         log: 'Updated a Cost Centre - Global List',
    //         method: 'update',
    //         subject: 'costcentre',
    ...
    public addLog(){
        ...
        let passData = {
            ...
            method: 'update',
            subject: 'costcentre',
            subjectID: this.data.costcentreID,
            prevdata: this.prevdata,
            data: this.editForm.value,
            url: window.location.href,
            userID: this.userDetails.user_id,
    ...
    public updateFBCostcentre(): void {
      ...
      this.data_api.updateFBCostcentre(this.data, this.editForm.value).then(() => {
          this.addLog();
...
@Component({
  selector: 'costcentres-deletedialog',
  templateUrl: 'costcentres-deletedialog.html',
  providers: [
    {provide: DateAdapter, useClass: AppDateAdapter},
    {provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS}
  ]
...
export class CostcentresDeleteDialog implements OnInit {
  ...
  constructor(
    private formBuilder: FormBuilder,
    public dialogRef: MatDialogRef<CostcentresDeleteDialog>,
    private data_api: DatasourceService,
    @Inject(MAT_DIALOG_DATA) public data) {}

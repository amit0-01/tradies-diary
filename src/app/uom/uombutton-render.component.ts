...
export class UomRenderComponent implements ViewCell, OnInit {
  ...
  openDialog(): void {
        console.log(this.renderValue);
        const dialogRef = this.dialog.open(UomDialog, {
            width: '400px',
    ...
    openDeleteDialog(): void {
      ...
      const dialogRef = this.dialog.open(UomDeleteDialog, {
          width: '400px',
          data: this.renderValue
      });
        ...
        console.log(result);
          ...
          if(result){ 
            ...
            this.spinnerService.show();   
                console.log(result);
                this.data_api.deleteFBUom(result).then(() => {
                  console.log('Delete new UOM Global List successfully!');
                  this.addLog();
                  $.notify({
                    icon: 'notifications',
                    message: 'Uom Deleted'
...
export class UomDialog implements OnInit {
    ...
    constructor(
      ...
      public dialogRef: MatDialogRef<UomDialog>,
    ...
    public addLog(){
        ...
        let passData = {
            todaysDate: today,
            log: 'Updated a UOM - Global List',
            method: 'update',
            subject: 'visitor',
            subjectID: this.data.uomID,
            prevdata: this.prevdata,
            data: this.editForm.value,
            url: window.location.href,
            userID: this.userDetails.user_id,
    ...
    public updateFBVisitor(): void {
      ...
      this.data_api.updateFBUom(this.data, this.editForm.value).then(() => {
          //this.dialogRef.close('success');
          this.addLog();
...
@Component({
  selector: 'uom-deletedialog',
  templateUrl: 'uom-deletedialog.html',
  providers: [
    {provide: DateAdapter, useClass: AppDateAdapter},
    {provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS}
  ]
...
export class UomDeleteDialog implements OnInit {
  ...
  constructor(
    private formBuilder: FormBuilder,
    private data_api: DatasourceService,
    public dialogRef: MatDialogRef<UomDeleteDialog>,
    @Inject(MAT_DIALOG_DATA) public data) {}

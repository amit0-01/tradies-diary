...
import { VariationsProjectDeleteDialog, VariationsProjectRenderComponent } from './variationsproject-render.component';
...
export class VariationsProjectComponent implements OnInit {
    ...
    ngOnInit() {        
      ...
      ngAfterViewInit(): void {
          $('#DataTables_Table_0').on('length.dt', (e, settings, len) => {
            ...
            changeTableLimit(this.data_api, this.currentUser.email, len,this.spinnerService, 'variation')
          });
        }
  ...
  loadProjects(){
    ...
    this.search_control_project.valueChanges
     ...
     projectSelect(event:any){
      ...
      this.router.navigate(['/variations/project/'+event.value]);
      setTimeout(()=>{
        this.getData();
      },300)
    }
  ...
  setTableData(source:any){
    ...
    this.dtOptions.columns  = [
      {
        ...
        createdCell: (cell: HTMLElement, cellData: any, rowData: any) => {
          ...
          cell.innerHTML = `
            <a style="color: #000000; cursor:pointer;" href="#/variations/project/${this.passID.id}/edit/${rowData.id}">
      ...
      { title: 'Variation No.', data: 'variantsNumber',
       },
      { title: 'Variation Name',data: 'variationsName',
      },
      { title: 'Total', data: null,
        render: (data)=>{
          return this.getTotal(data.variationGroupArray).toFixed(2);
        }
       },
    ...
    this.dtTrigger.next();
    getTableLimit(this.data_api,this.currentUser.email, 'variation').then(res =>{
          if(res){
...
// delete variation
 openDeleteDialog(rowData:any): void {
      ...
      const dialogRef = this.dialog.open(VariationsProjectDeleteDialog, {
          width: '400px',
          data: rowData
      });
        ...
        console.log(result);
          ...
          if(result){
                ...
                if(result.fest_confirm=='DELETE'){ 
                    ...
                    for (let group of result.variationGroupArray) { 
                    ...
                    this.data_api.deleteFBVariation(result.id).then(() => {
                      this.spinnerService.hide();  
                      // this.addLog();
                      $.notify({
                        icon: 'notifications',

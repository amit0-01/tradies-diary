...
import { first , takeUntil, startWith, finalize } from 'rxjs/operators';
...
export class VariationsProjectEditComponent implements OnInit {
      ...
      this.data_api.getFBProject(this.passID.id).pipe(first()).subscribe(data => {
          ...
          // }
              ...
              private updateEmailList(emailList: string[], type: 'cc' | 'bcc') {
                ...
                isVariationEditable = false;  // Initially set to false

          // Toggle the edit mode for the project variation recipients
            toggleVariationRecipientEdit() {
              this.isVariationEditable = !this.isVariationEditable;

              // If switching to editable mode, enable the control
              if (this.isVariationEditable) {
    ...
    onToppingRemoved(topping, index: number) {
      ...
      // console.log('this.pojrectoownerContro', this.projectOwnerControl)
      // const userIds = this.projectOwnerControl.value.map(user => user.id);
      // localStorage.setItem('projectOwnerControl', JSON.stringify(userIds))
    }
  ...
  sendClientEmail(variantID,projectID){
      ...
      // ADMIN EMAILS SENT TO CC
      let emailsCC = [...this.projectData.VariationEmailCC];
      if(emailsCC){
      ...
      let emailsBCC = [...this.projectData.VariationEmailBCC]
      if(emailsBCC){
        ...
        try {
          ...
          updateRecipients(
            ...
            (res: any) => {
              ...
              this.getProject();
              console.log('this.projectVariationRecipientControl', this.projectVariationRecipientControl);
            },
            (error: any) => {
              // Error callback: Handle error if something goes wrong
              console.error('Error during Varition update:', error);
            }
          );
        } catch (error) {
  ...
  btnFunctionList(){
    if(this.addFestForm.dirty){
      ...
      console.log("this.passid.id", this.passID.id)
      this.router.navigate(['/variations/project/'+this.passID.id]);
    }
